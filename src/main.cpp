...
int main(int argc, char* argv[]) {
    ...
    raw::shader object_shader("shaders/objects/vertex_shader.glsl",
    ...
    for (int i = 0; i < AM_POINT_LIGHTS; ++i) {
        ...
        object_shader.set_vec3("point_lights[" + std::to_string(i) + "].specular", 1.0f, 1.0f, 1.0f);
    ...
    while (running) {
        ...
        for(unsigned int i = 0; i < 2; i++)
        {
            ...
            if (i == 0) {
                model = model * glm::mat4_cast(object_quat);
            }
            object_shader.set_mat4("model", glm::value_ptr(model));
            glDrawArrays(GL_TRIANGLES, 0, 36);
        }

        // render platform
        glm::mat4 platform_model = glm::mat4(1.0f);
        platform_model = glm::translate(platform_model, glm::vec3(0.0f, -2.0f, 0.0f));
        platform_model = glm::scale(platform_model, glm::vec3(15.0f, 0.2f, 15.0f));
        object_shader.set_mat4("model", glm::value_ptr(platform_model));
        glDrawArrays(GL_TRIANGLES, 0, 36);
