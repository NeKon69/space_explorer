 ...
 */
std::enable_if_t<!std::is_array_v<T>, std::shared_ptr<T> > make_shared(Args &&...args) {
	...
	try {
		...
		constructed_hub = new (raw_block + offsetof(combined<T>, hub_ptr))
			hub(constructed_ptr, raw_block, deleters::destroy_make_shared_object<T>,
				deleters::deallocate_make_shared_block);
		constructed_ptr =
...
std::enable_if_t<std::is_array_v<T>, std::shared_ptr<T> > make_shared(size_t size) {
	...
	try {
		constructed_hub = new (raw_block)
			raw::hub(nullptr, raw_block, &deleters::destroy_make_shared_array<element_type>,
					 &deleters::deallocate_make_shared_block, size);
