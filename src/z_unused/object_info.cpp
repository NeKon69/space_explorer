...
namespace raw::z_unused {
	object_info::object_info(std::shared_ptr<z_unused::object> obj) : object(obj) {
	}

	object_info::object_info(std::shared_ptr<z_unused::object> obj, glm::vec3 position)
		: object(obj), transform(glm::translate(transform, position)) {
	}

	object_info::object_info(std::shared_ptr<z_unused::object> obj, glm::vec3 position, glm::vec3 scale)
		: object(obj), transform(glm::translate(glm::scale(transform, position), scale)) {
	}

	object_info::object_info(std::shared_ptr<z_unused::object> obj, glm::mat4 transformation)
