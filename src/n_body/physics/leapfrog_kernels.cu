...
__device__ void compute_kick(space_object<T> *objects, uint16_t count, uint16_t current, T g,
    ...
    template<typename T>
__global__ void compute_leapfrog(space_object<T> *objects, glm::mat4 *objects_model, uint16_t count,
                                 T dt, T g) {
    ...
    template __global__ void compute_leapfrog<float>(space_object<float> *objects, glm::mat4 *,
                                                     uint16_t count, float dt, float g);

    template __global__ void compute_leapfrog<double>(space_object<double> *, glm::mat4 *,
