...
__global__ void compute_k_final(graphics::instanced_data *data, space_object_data<T> *objects,
	...
	printf("Position: x = %f, y = %f, z = %f\n", objects[x].position.x, objects[x].position.y,
		   objects[x].position.z);
	printf("Radius: %f\n", objects[x].radius);
	data[x].model =
		glm::scale(glm::translate(glm::mat4(1.0f), static_cast<glm::vec3>(objects[x].position)),
				   glm::vec3(objects[x].radius));
	print_matrix_device(data[x].model, "Object Position");
}
...
template
__global__ void compute_d<float>(space_object_data<float>* objects, uint16_t count, float dt);
template
__global__ void compute_d<double>(space_object_data<double>* objects, uint16_t count, double dt);
