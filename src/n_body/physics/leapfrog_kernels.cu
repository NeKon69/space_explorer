...
template<typename T>
__device__ void compute_kick(physics_component<T> *objects, uint16_t count, uint16_t current, T g,
							 T epsilon, T dt) {
	auto				 a_total		= glm::vec<3, T>(0.0);
	physics_component<T> current_object = objects[current];
	...
	for (uint16_t i = 0; i < count; ++i) {
		...
		physics_component<T> local_object	= objects[i];
...
template<typename T>
__global__ void compute_k(graphics::instanced_data *data, physics_component<T> *objects,
...
template<typename T>
__global__ void compute_d(physics_component<T> *objects, uint16_t count, T dt) {
	const uint16_t x = blockIdx.x * blockDim.x + threadIdx.x;
	if (x >= count) {
		return;
	}
...
template<typename T>
__global__ void compute_k_final(graphics::instanced_data *data, physics_component<T> *objects,
								uint16_t count, T dt, T g, T epsilon) {
	const uint16_t x = blockIdx.x * blockDim.x + threadIdx.x;
	if (x >= count) {
		return;
...
template __global__ void compute_k<float>(graphics::instanced_data *data,
										  physics_component<float> *objects, uint16_t count,
...
template __global__ void compute_d<float>(physics_component<float> *objects, uint16_t count,
										  float dt);
template __global__ void compute_d<double>(physics_component<double> *objects, uint16_t count,
										   double dt);
