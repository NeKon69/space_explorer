...
//namespace raw {
//
//void event_handler::_setup_keys(raw::playing_state* scene) {
//	// expand this when you add shaders/uniforms
//	auto update_all_view_uniforms = [scene]() {
//		update_uniform_for_shaders("view", scene->camera.view(), scene->renderer.get_all_shaders());
//		update_uniform(scene->renderer.get_basic_shader(), "sp_light.position",
//					   scene->camera.pos());
//		update_uniform(scene->renderer.get_basic_shader(), "sp_light.direction",
//					   scene->camera.front());
//	};
//	// I am in hell
...
//			if (!(time_since_epoch < raw::time(1000.f / predef::UPDATE_MOVE_TIME))) {
...
//		scene->renderer.get_basic_shader()->use();
//		scene->renderer.set_dir_light(!scene->renderer.get_dir_light());
//		update_uniform(scene->renderer.get_basic_shader(), "need_dir_light",
//					   scene->renderer.get_dir_light());
//	};
//
...
//}
//
//void event_handler::setup(raw::playing_state* scene) {
...
//								   scene->renderer.get_all_shaders());
//	};
//	_setup_keys(scene);
//	// pretty easy, right?
//	events[SDL_EVENT_KEY_DOWN] = [this](const SDL_Event& event) {
//		if (buttons.contains(event.key.scancode)) {
//			buttons[event.key.scancode].press();
//		}
//	};
//	events[SDL_EVENT_QUIT] = [this](const SDL_Event&) {
//		if (buttons.contains(SDL_SCANCODE_ESCAPE)) {
//			buttons[SDL_SCANCODE_ESCAPE].press();
//		}
//	};
//	events[SDL_EVENT_MOUSE_MOTION] = [scene, update_front_view_uniforms](const SDL_Event& event) {
...
//	};
//	events[SDL_EVENT_KEY_UP] = [this](const SDL_Event& event) {
//		if (buttons.contains(event.key.scancode)) {
//			buttons[event.key.scancode].release();
//		}
//	};
//	events[SDL_EVENT_MOUSE_WHEEL] = [scene, update_projection_uniforms](const SDL_Event& event) {
