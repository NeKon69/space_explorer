...
namespace raw::core::camera {
	player_controller::player_controller(core::camera::camera &cam) : camera(cam), velocity(1.0f) {
	}

	void player_controller::update(const raw::core::camera::movement_state &state, float delta_time) {
		delta_time /= 1000;
